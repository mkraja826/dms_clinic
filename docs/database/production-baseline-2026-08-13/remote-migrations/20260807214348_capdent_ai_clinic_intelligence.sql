...
begin
  ...
  select jsonb_build_object(
    ...
    'doctor_performance', coalesce((select jsonb_agg(x) from (
      select pr.id doctor_id,pr.name,
        count(pv.id) filter (where pv.visit_date>=v_month_start and pv.visit_date<v_next_month_start) visits_this_month,
        count(distinct pv.patient_id) filter (where pv.visit_date>=v_month_start and pv.visit_date<v_next_month_start) unique_patients_this_month,
        count(a.id) filter (where a.appointment_time>=v_month_start and a.appointment_time<v_next_month_start) appointments_this_month
      from profiles pr
      left join patient_visits pv on pv.doctor_id=pr.id and pv.clinic_id=v_clinic
      left join appointments a on a.doctor_id=pr.id and a.clinic_id=v_clinic
      where pr.clinic_id=v_clinic and pr.active=true and pr.role in ('owner','head_doctor','doctor')
      group by pr.id,pr.name order by visits_this_month desc
    ) x),'[]'::jsonb),
...
grant execute on function public.get_capdent_ai_clinic_intelligence() to authenticated;
