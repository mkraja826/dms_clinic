...
begin
  ...
  user_email := coalesce(nullif(auth.jwt() ->> 'email', ''), nullif(trim(clinic_email), ''));
  ...
  values (auth.uid(), new_clinic_id, trim(owner_name), user_email, 'owner', true)
...
drop function if exists public.create_owner_clinic(text, text, text, text, text);
...
comment on function public.create_owner_clinic(
  ...
  time without time zone,
  time without time zone
) is 'Creates an owner clinic, free subscription, and owner profile. One function supports legacy 5-field and current 9-field clients through defaults.';
